...
def _master_bounds(solution) -> dict[str, object] | None:
    """Return the reproducible MILP bound record written into the paper package."""
    diagnostics = solution.diagnostics.get("q2_master")
    if not diagnostics:
        return None
    incumbent = float(diagnostics["primary_objective"])
    dual_bound = diagnostics.get("primary_dual_bound")
    return {
        "primary_incumbent_minutes": incumbent,
        "primary_dual_bound_minutes": dual_bound,
        "primary_mip_gap": diagnostics.get("primary_mip_gap"),
        "primary_mip_gap_percent": (
            100.0 * float(diagnostics["primary_mip_gap"])
            if diagnostics.get("primary_mip_gap") is not None
            else None
        ),
        "primary_status": diagnostics.get("primary_status"),
        "primary_proven_optimal": diagnostics.get("primary_proven_optimal"),
        "secondary_status": diagnostics.get("secondary_status"),
        "secondary_mip_gap": diagnostics.get("secondary_mip_gap"),
        "secondary_proven_optimal": diagnostics.get("secondary_proven_optimal"),
        "lexicographic_weights": diagnostics.get("lexicographic_weights"),
        "final_objectives": diagnostics.get("final_objectives"),
        "candidate_sequences": diagnostics.get("candidate_sequences"),
        "candidate_variants": diagnostics.get("candidate_variants"),
        "compatible_assignments": diagnostics.get("compatible_assignments"),
...
def main() -> int:
        ...
        write_json(
            run_dir / "run_config.json",
            {
                ...
                "master_config": {
                    ...
                    "time_limit_seconds": args.time_limit,
                    "mip_relative_gap": args.mip_gap,
                    "primary_fraction": master_config.primary_fraction,
                },
    ...
    write_json(
        run_dir / "q2-bounds.json",
        {
            "pair_master": _master_bounds(pair_solution),
            "expanded_master": (
                _master_bounds(expanded_solution) if expanded_solution else None
            ),
            "status_definition": {
                "0": "在给定候选池上证明最优",
                "1": "达到时间或迭代限制，保留当前最好可行解与对偶下界",
                "2": "判定不可行",
                "3": "判定无界",
                "4": "数值或其他求解器错误",
            },
