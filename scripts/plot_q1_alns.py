...
def main() -> int:
    ...
    axes[0].step(x, best_time, where="post", linewidth=1.8, label="Best aircraft time")
    axes[0].axhline(args.lower_bound, color="0.45", linestyle="--", linewidth=1.1, label="Lower bound")
    axes[0].set_ylabel("Aircraft time (min)")
    axes[0].grid(alpha=0.25)
    axes[0].legend(frameon=False, ncol=2)
    axes[1].step(x, best_flights, where="post", linewidth=1.6, color="#d97706")
    axes[1].set_xlabel("Global ALNS iteration")
