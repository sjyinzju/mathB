...
> Exact certification update：当前 14,730 仍是 validated UB。现已严格证明它是最终 fully-priced root 1,041-column master 的整数最优，并建立 full-space/tree global lower bound 14,090.327485380118（ceil 14,091）。Branch-aware pricing 已认证；fast exact pricing oracle 已认证（Gate 8.2 36/36，`Q1_FAST_PRICING_REPORT.md`）并恢复 checkpoint：累计 10 个递归树节点 fully priced、14+ open nodes，global LB 仍停在 14,090.327（arc branching 界增益退化），无任何 fathom。因此全局状态为 **BRANCH_AND_PRICE_INCOMPLETE**；不得称 14,730 为 global optimum。
