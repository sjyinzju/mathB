...
def grouped_q2_splits(run_rows: Iterable[dict[str, object]]) -> dict[str, str]:
    run_ids = sorted({str(row["run_id"]) for row in run_rows})
    if not run_ids:
        return {}
    if len(run_ids) == 1:
        return {run_ids[0]: "train"}
    if len(run_ids) == 2:
        return {run_ids[0]: "train", run_ids[1]: "test"}
    ordered = sorted(
        run_ids,
    ...
    for index, run_id in enumerate(ordered):
        result[run_id] = ("validation", "test", "train")[index % 3]
    return result
...
def build_q2_learning_dataset(
    ...
    split_rows = [
        {"run_id": row["run_id"], "seed": row["seed"], "split": splits[str(row["run_id"])]}
        for row in run_rows
    ]
