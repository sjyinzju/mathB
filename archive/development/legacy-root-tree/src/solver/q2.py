...
def _solve_master_arrays(
    ...
    primary_limit = config.time_limit_seconds * (1.0 - config.secondary_fraction)
    first = milp(
        c=primary,
        ...
        options={
            "time_limit": max(1.0, primary_limit),
    ...
    for variant_id, variant in enumerate(variants):
        secondary[variant_id] = (
            1_000.0 + variant.evaluation.total_fuel_consumption_kg * 1.0e-3
        )
    for pair, column in x_column.items():
        secondary[column] = compatible[pair][2] * 1_000_000.0
    second = milp(
        c=secondary,
        integrality=integrality,
        bounds=bounds,
        constraints=constraints,
        options={
            "time_limit": max(1.0, config.time_limit_seconds * config.secondary_fraction),
            "mip_rel_gap": config.mip_relative_gap,
            "presolve": True,
        },
    )
